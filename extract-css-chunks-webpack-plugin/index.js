...
var plugin = {name: 'ExtractCSSTextPlugin'};
var nextId = 0;
...
ExtractTextPlugin.prototype.renderExtractedChunk = function(compilation, chunk) {
  var that = this;
  var source = new ConcatSource();

  forEachChunkModule(chunk, function(chunkModule) {
    //	chunk.modulesIterable.forEach(function(chunkModule) {
    var moduleSource = chunkModule.source(
...
ExtractTextPlugin.prototype.mergeNonInitialChunks = function(chunk, intoChunk, checkedChunks) {
  if (!intoChunk) {
    var newCheckedChunks = [];

    for (var asyncChunk of chunk.getAllAsyncChunks()) {
    ...
    var newCheckedChunks = checkedChunks.concat(chunk);

    for (var chunkModule of chunk.modulesIterable) {
...
function ExtractTextPlugin(options) {
  ...
  this.filename = options.filename || (process.env.NODE_ENV === 'development' ? '[name].css' : '[name].[hash].css');
...
ExtractTextPlugin.prototype.apply = function(compiler) {
  ...
  compiler.hooks.thisCompilation.tap(plugin, function(compilation) {
    ...
    compilation.hooks.normalModuleLoader.tap(plugin, function(loaderContext, module) {
      ...
      loaderContext[NS] = function(content, opt) {
        ...
        return  module[NS + "/extract"]; // eslint-disable-line
    ...
    compilation.hooks.optimizeTree.tapAsync(plugin, function(chunks, modules, callback) {
      ...
      chunks.forEach(function(chunk, i) {
        ...
        /*	for (const chunkGroup of chunk.groupsIterable) {
                ...
                //if(chunkGroup.entrypoints)
                //extractedChunk.entrypoints = chunkGroup.entrypoints;
          }
            chunk.groupsIterable.forEach(function(chunkGroup) {
          extractedChunk.addGroup(chunkGroup);
          })
        for (const chunkGroup of chunk.groupsIterable) {
                extractedChunk.addGroup(chunkGroup);
      ...
      async.forEach(chunks, function(chunk, chunkCallback) {
        ...
        async.forEach(Array.from(chunk.modulesIterable).sort(
          (a, b) => a.index - b.index
          ),
          function(module, moduleCallback) {
            var meta = module[NS];
            ...
            if (err) return chunkCallback(err);
            chunkCallback();
          });
      }, function(err) {
    ...
    if (process.env.NODE_ENV === 'development') {
      compilation.hooks.beforeChunkAssets.tap(plugin, function() {
...
//if(module.getOriginalModule){
            ...
            originalModule._source._value =
originalModule._source._value.replace(/%%extracted-file%%/g, file);
            //}
          });
    ...
    compilation.hooks.additionalAssets.tapAsync(plugin, function(callback) {
      ...
      extractedChunks.forEach(function(extractedChunk) {
        if (extractedChunk.getNumberOfModules()) {
          ...
          var stylesheet = this.renderExtractedChunk(compilation, extractedChunk);
...
function forEachChunkModule(chunk, cb) {
  ...
  if (chunk.modulesIterable) {
    Array.from(chunk.modulesIterable).forEach(cb)
