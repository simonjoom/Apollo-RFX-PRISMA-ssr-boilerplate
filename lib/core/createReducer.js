...
export default (initialState: Object, routes: Routes) => (
  ...
  if (l && l.kind === 'set') {
    const merg = Object.assign({}, { ...st.location }, { ...l })
    const { commit, location: { kind, ...location }, ...act } = action
    return { ...st, ...act, ...location , commit }
  }

  if (r && l && r.path && (l.url !== st.url || /load|reset/.test(l.kind))) {
    const merg = Object.assign({}, { ...st.location }, { ...l })
    console.log("actionmerg", merg, st)
    const { type, params, query, state, hash, basename } = action
    const { universal } = st
    const s = { type, params, query, state, hash, basename, universal, ...action.location }
