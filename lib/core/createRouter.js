...
function te(){
  var ff=getState();
  console.log("getlocattion",ff.location)
  return ff|| {}
}

    const getLocation = (s) => selectLocationState(s || te())
    ...
    global.rudy = api // make rudy available via `context` with no extra Providers, (see <Link />)
